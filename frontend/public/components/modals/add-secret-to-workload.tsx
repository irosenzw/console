...
import { withExtensions, SecretExtention, isSecretExtension } from '@console/plugin-sdk';
import { getRandomChars } from '@console/shared';

const workloadResourceModels = [
  DeploymentModel,
...
export const AddSecretToWorkloadModal = withExtensions<SecretExtensionProps>({
  secretExtensions: isSecretExtension,
})(
  ...
  > {
    ...
    onWorkloadChange = (selectedWorkloadUID: string) => {
      ...
      const isVMWorkload = model === VirtualMachineModel;
      let vmTemplate = null;
      let serialNumber: string;
      let serialErrMsg: string;
      const onSecretWorkloadChange = this.props.secretExtensions
        .filter((s) => s.properties.onSecretWorkloadChange)
        .map((s) => s.properties.onSecretWorkloadChange)[0];
      if (isVMWorkload) {
        onSecretWorkloadChange(obj)
          .then((value) => {
            vmTemplate = value;
          })
          .catch(() => {
          ...
          (serialNumber = this.getExistingEnvDiskSerial(obj));
        if (serialNumber) {
    ...
    getExistingEnvDiskSerial = (vm: K8sResourceCommon): string => {
      let serialNumber: string;
      this.props.secretExtensions.forEach((s) => {
        serialNumber = s.properties.getEnvDiskSerial(vm, this.props.secretName);
      });
      return serialNumber;
    };
    ...
    getVMEnvDiskPatches = (vm: K8sResourceCommon) => {
      const getVMEnvDiskPatches = this.props.secretExtensions
        .filter((s) => s.properties.getVMEnvDiskPatches)
        .map((s) => s.properties.getVMEnvDiskPatches)[0];

      return getVMEnvDiskPatches(
        vm,
    ...
    render() {
      ...
      const serialErrMsg =
        addAsDisk &&
        this.getSerialErrorMsg(this.getExistingEnvDiskSerial(workloadOptions[selectedWorkloadUID]));
      ...
      return (
        ...
        >
          ...
          <ModalSubmitFooter
            ...
            submitDisabled={addAsDisk && !!serialErrMsg}
          />
        </form>
      );
    }
...
type SecretExtensionProps = {
  secretExtensions: SecretExtention[];
};
